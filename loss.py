...
class Our_SupCL_loss(nn.Module):
    ...
    def forward(self, features, logits, labels, index, delta_smooth, epoch):
        ...
        low_uncertain_mask = low_unc.view(-1,1)&low_unc.view(1,-1)
        ...
        log_prob = logits - torch.log(exp_logits.sum(1, keepdim=True)+tmp)
...
class CE_OurLoss(nn.Module):
    ...
    def forward(self, logits, targets):
        ...
        if len(targets.size())==1:
            target_oh = torch.zeros(targets.size(0), self.c).to(self.device).scatter_(1, targets.view(-1,1), 1) # convert label to one-hot
        # TODO for debug
        # print(logits.min(), logits.max(), logits_norm.min())
        loss = -torch.mean(torch.sum(torch.log_softmax(logits, dim=1)[:,:-1] * target_oh, dim=1))
